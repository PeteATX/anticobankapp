...
public class Menu {
	...
	private static Superallusers su;
	boolean exit;
...
private void createAccount() {
	 ...
	 while(!valid) {
	    ...
	    Superallusers su = new Superallusers(valid, userName, password);
	      SuperBankRepositoryImpl sr = new SuperBankRepositoryImpl();
	      sr.createUsers(su);
}
...
private void login(Scanner sc) {
	    ...
	    while (loginInput) {
	  	...
	  	input = input.trim();
	  	if (input.equals(su.getPassword())) {
	  		loginInput= false;
	  		su.setloggedIn(true);   //Have to create
	  		System.out.println("You are now logged into the Bank of Knuts.");
	  	} else {
	  		System.out.println("Incorrect password. Please try again or press 'H' to go home.");
	  	}	
	  	}
 ...
 * SuperPetService superPetService = new SuperPetService();
			SuperPet superPet = new SuperPet(4, "Frrrrr", "Power?", "Color?", 67, "Greatest SuperOwner");
